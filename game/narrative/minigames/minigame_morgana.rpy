...
#
# Normal mode: sequence starts at 4, caps at 8. One correction allowed per round.
# Hard mode:   sequence starts at 6, caps at 10. Zero corrections. Look-alike pairs.
#
# Rewards
#   Normal pass:   +2 INT, +100g
#   Normal fail:   +1 INT
#   Hard pass:     +2 INT, +2 INT bonus, +150g  → triggers morgana_reward_scene
#   Hard fail:     +1 INT
#
...
init python:
    ...
    MORGANA_INGREDIENTS_NORMAL = [
        "Carrot", "Onion", "Mushroom", "Herb Bundle",
        "Fish", "Salt Jar",
    ]

    # Hard mode adds look-alikes
    MORGANA_INGREDIENTS_HARD = [
        "Carrot", "Onion", "Mushroom", "Herb Bundle",
        "Fish", "Salt Jar", "Dried Herb", "Root Spice",
    ]

    def morgana_seq_length(visits, hard_mode=False):
        base = 6 if hard_mode else 4
        cap  = 10 if hard_mode else 8
            ...
            hbox:
                ...
                for ing in shuffled:
                    if ing not in entered:
                        imagebutton:
                            idle  "bt morgana {}.png".format(ing.lower().replace(" ", "_"))
                            hover "bt morgana {}.png".format(ing.lower().replace(" ", "_"))
...
label morgana_job:
    ...
    while len(_mo_entered) < len(_mo_seq) and not _mo_reset:
        ...
        # Screen calls morgana_tap() which sets _mo_tap_result
...
label morgana_job_hard:
    ...
    n "Memorize carefully. Some of these look similar."
    $ _mo_idx = 0
    while _mo_idx < len(_mo_seq):
        show screen morgana_memorize(_mo_seq, _mo_idx)
        $ renpy.pause(1.5)
